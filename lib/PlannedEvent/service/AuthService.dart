...
class AuthService {
  ...
  Future<Map<String, dynamic>?> login() async {
    try {
      final authUrl = '$authority/oauth2/v2.0/authorize?client_id=$clientId'
          '&response_type=code'
          '&redirect_uri=${Uri.encodeComponent(redirectUri)}'
          '&response_mode=query'
          '&scope=${Uri.encodeComponent(_scopes.join(' '))}'
  ...
  Future<bool> setWorkGroups(int userId, List<int> workGroupIds) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to set workgroups: ${response.statusCode} ${response.body}');
      }
    } catch (e) {
      throw Exception('Error setting workgroups: $e');
    }
  ...
  Future<Map<String, dynamic>?>
      getCurrentUserWorkgroupsWithPermissions() async {
    final userId = await getStoredUserId();
    if (userId == null) return null;
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
            '$baseUrl/api/users/$userId/current-user-workgroups-with-permissions'),
        headers: headers,
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var id in workGroupIds) {
          ...
          if (workGroupResponse.statusCode == 200) {
            final workGroup =
                WorkGroup.fromJson(jsonDecode(workGroupResponse.body));
            workGroupNames.add(workGroup.name);
          } else {
            throw Exception(
                'Failed to fetch workgroup $id: ${workGroupResponse.statusCode}');
          }
        }
        ...
        throw Exception(
            'Failed to fetch workgroup IDs: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Error fetching workgroups: $e');
    }
