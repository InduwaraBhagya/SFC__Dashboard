...
class RegularRecordScreen extends StatefulWidget {
  ...
  const RegularRecordScreen(
      {super.key, this.workgroupName, required this.user, this.onBack});
...
class _RegularRecordScreenState extends State<RegularRecordScreen>
  ...
  Future<Map<String, dynamic>> _fetchRecords({int page = 1}) async {
    ...
    try {
      ...
      final List<RegularRecord> newRecords =
          (result['records'] as List<dynamic>?)?.cast<RegularRecord>() ?? [];
      final int totalCount = result['totalCount'] as int? ?? 0;
      final int totalPages = result['totalPages'] as int? ?? 1;

      if (kDebugMode) {
        print(
            'Fetched ${newRecords.length} records, page: $page, total: $totalCount, totalPages: $totalPages');
  ...
  List<RegularRecord> _getFilteredRecords() {
    ...
    if (_soSearchController.text.isNotEmpty) {
      ...
      filtered = filtered
          .where((r) => r.soNumber?.toLowerCase().contains(query) ?? false)
          .toList();
    }
    ...
    if (!_filterAll) {
      if (_filterInProgress) {
        ...
        filtered = filtered
            .where((r) => r.woStatus?.contains('Completed') ?? false)
            .toList();
      }
    }
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    RegularRecordDetailsScreen(record: record)));
      },
      child: Container(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('No regular records found for:',
              style: GoogleFonts.outfit(
                  fontSize: 14, color: Colors.grey.shade500)),
          Text(_workgroupName ?? 'All Workgroups',
              style: GoogleFonts.outfit(
                  fontSize: 16,
