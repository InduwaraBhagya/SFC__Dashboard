...
class OLAViolateRecordService {
  ...
  Future<Map<String, dynamic>> fetchOLAViolateRecords({
    ...
    try {
      ...
      final url = Uri.parse('$baseUrl/api/somsdashboard/records/${Uri.encodeComponent(wgName)}/olaViolated');
  ...
  Future<OLAViolateRecord?> fetchPlannedEventDetails(int recordId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result['records']?['\$values'] != null && result['records']['\$values'].isNotEmpty) {
